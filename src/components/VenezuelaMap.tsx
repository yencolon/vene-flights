...
const toPolygons = (geom: GeoFeature['geometry']): Polygon[] => {
  ...
  if (geom.type === 'MultiPolygon') return geom.coordinates as unknown as Polygon[]
  return []
}

const venezuelaPolygons = toPolygons(venezuelaGeo.geometry as GeoFeature['geometry'])
...
const makeOrthographic = (center: LonLat, R: number, isGlobe: boolean): Projector => {
...
const angularDist = (a: LonLat, b: LonLat): number => {
  const phiA = a[1] * DEG, phiB = b[1] * DEG
  const dlam = (b[0] - a[0]) * DEG
  const cosc = Math.sin(phiA) * Math.sin(phiB) + Math.cos(phiA) * Math.cos(phiB) * Math.cos(dlam)
...
const polygonsToClosedPath = (polys: ReadonlyArray<Polygon>, project: Projector['project']) =>
  polys.flatMap((poly) => poly.map((ring) => ringToClosedPath(ring, project))).join(' ')

const polygonsToOpenPath = (polys: ReadonlyArray<Polygon>, project: Projector['project']) =>
  polys.flatMap((poly) => poly.map((ring) => ringToOpenPath(ring, project))).join(' ')
...
const greatCircleSegments = (
  ...
  const phi1 = p1[1] * DEG, lam1 = p1[0] * DEG
  const phi2 = p2[1] * DEG, lam2 = p2[0] * DEG
...
export function VenezuelaMap({
  ...
  const [hoveredAirportIcao, setHoveredAirportIcao] = useState<string | null>(null)
  ...
  useLayoutEffect(() => {
    ...
    const fd = flightsByIcao[selectedIcao]
    if (!airport || !fd) {
      setZoomXform({ s: 1, x: 0, y: 0 })
      return
    }
  ...
  const selectedAirport = useMemo(
    () => (selectedIcao ? airports.find((a) => a.icao === selectedIcao) ?? null : null),
    [selectedIcao],
  )
  ...
  useEffect(() => {
    ...
    fetch(`${import.meta.env.BASE_URL}world-110m.geojson`)
      ...
      .then((d) => { if (!cancelled) setWorldData(d) })
  ...
  const projector = useMemo<Projector>(() => {
    if (selectedAirport && flightData) {
      ...
      // Force globe mode whenever any route leaves Venezuela so the disc
      // outline always frames international hops, even short ones (e.g.
      // Maracaibo→Miami) that don't pass the angular-spread threshold.
      const { R, isGlobe } = hasInternational
        ? { R: 0.43 * VIEW, isGlobe: true }
        : computeScale(baseCenter, points)
  ...
  }, [selectedAirport, flightData, rotation])
  ...
  const activeDest =
    filteredFlightData?.destinations.find((d) => d.airport_code === activeDestCode) ?? null
  ...
  // render the selected one to keep things readable.
  const visibleAirports = projector.isGlobe
    ? airports.filter((a) => a.icao === selectedIcao)
    : airports
  ...
  return (
      <figure class="map-figure">
        {selectedAirport && (
          <button
            type="button"
            class="map-back-btn"
            onClick={() => { onSelect(null); setHoveredDestCode(null) }}
          >
            ← Back to Venezuela
          </button>
        )}
      ...
      <svg
        ...
        aria-label={
          selectedAirport
            ? `Routes from ${selectedAirport.name}`
      ...
      >
        ...
        {projector.isGlobe && (
          <rect
            ...
            onPointerMove={(e) => {
              ...
              const clampedLat = Math.max(-85, Math.min(85, baseLat + newLatOffset))
        ...
        <g clip-path="url(#globe-clip)">
         <g transform={`translate(${zoomXform.x} ${zoomXform.y}) scale(${zoomXform.s})`}>
          ...
          {filteredFlightData && selectedAirport &&
            filteredFlightData.destinations.map((dest) => {
              ...
              for (const leg of dest.flights) {
                ...
                opTotals.set(leg.operator, prev + numericFreq(leg.weekly_frequency))
              }
              ...
              return (
                <g
                  key={`route-${dest.airport_code}`}
                  class={cls.join(' ')}
                  tabIndex={0}
                  role="button"
                  aria-label={`Route to ${dest.city}`}
                ...
                >
                  <path d={baseD} class="route-hit" style={`stroke-width: ${hitWidth}px`} />
          ...
          {filteredFlightData && filteredFlightData.destinations.map((dest) => {
          const c = destinationCoords[dest.airport_code]
          if (!c) return null
          ...
          return (
            ...
            >
              ...
              {(showLabels || isActive) && (
                <text x={px + 7 * dotScale} y={py - 6 * dotScale} class="destination-label">{dest.city}</text>
              )}
            </g>
          )
        })}
        ...
        {visibleAirports.map((airport) => {
          ...
          return (
            ...
            >
              ...
              {showLabel && (
                <text x={px + 10 * dotScale} y={py - 8 * dotScale} class="airport-label">
        ...
        <div class="map-hud" aria-live="polite">
          {activeDest ? (
                ...
                {activeDest.flights.map((leg, i) => (
                  <li key={i}>
                    <div class="hud-leg-row">
                      ...
                      <span class="hud-freq">{formatFrequency(leg.weekly_frequency)}</span>
                    </div>
            ...
            <>
              ...
              <div class="hud-stats">
                <div><strong>{filteredFlightData.statistics.total_weekly_flights}</strong><span>weekly flights</span></div>
                <div><strong>{filteredFlightData.destinations.length}</strong><span>destinations</span></div>
                <div><strong>{filteredFlightData.statistics.flights_to_start}</strong><span>upcoming</span></div>
              </div>
              <p class="hud-hint">Hover a route or click one to lock its details.</p>
            </>
          ) : (
            <>
              <div class="hud-title">{selectedAirport.name}</div>
              <p class="hud-hint">No flight data available yet for this airport.</p>
            </>
          )}
        </div>
      )}
